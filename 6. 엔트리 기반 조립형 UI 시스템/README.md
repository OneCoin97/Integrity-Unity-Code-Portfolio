...
UI마다 텍스트, 이미지, 게이지와 버튼을 전용 스크립트에서 직접 연결하면 비슷한 화면이 늘어날수록 데이터 전달 코드와 화면별 관리 코드가 반복됩니다. 이를 줄이기 위해 표시할 값과 동작을 타입별 `UIEntry`로 분리하고, `UIView` 아래에 필요한 Entry를 배치하는 것만으로 하나의 화면을 조립하도록 구성했습니다.

화면을 사용하는 외부 시스템은 UI 컴포넌트의 세부 구현을 직접 제어하지 않고 `UIViewEntryInput`에 필요한 데이터를 담아 전달합니다. 각 Entry는 문자열, 숫자, Sprite, 색상이나 Action 중 자신이 처리하는 타입만 받아 텍스트, 이미지, 게이지와 버튼 동작으로 표현합니다.
