...
1. `GameManager.cs` — 전투 시작·종료와 스테이지 이동처럼 사건별 이벤트 채널을 소유하고 등록·해제·실행 API를 제공하는 최상위 이벤트 오케스트라
...
`GameManager.cs`의 공개 API를 먼저 확인한 뒤 `GameEventSubscriptions.cs`, `GameEventSubscription.cs`, `GameModeEvent.cs` 순서로 보면 사건별 이벤트 채널, 구독 목록, 개별 구독 정보와 상태 연결의 책임을 빠르게 파악할 수 있습니다.
